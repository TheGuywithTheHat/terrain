...
float newNoise(float x, float y) {
  ...
  return newLerp(v, newLerp(u, grad(p[AA], x    , y    ),
                               grad(p[BA], x - 1, y    )),
                    newLerp(u, grad(p[AB], x    , y - 1),
                               grad(p[BB], x - 1, y - 1)));
}
...
float newLerp(float t, float a, float b) {
  return a + t * (b - a);
}
...
/*float newNoise(float x, float y, float z) {
  ...
  return newLerp(w, newLerp(v, newLerp(u, grad(p[AA    ], x    , y    , z     ),  // AND ADD
                                          grad(p[BA    ], x - 1, y    , z     )), // BLENDED
                               newLerp(u, grad(p[AB    ], x    , y - 1, z     ),  // RESULTS
                                          grad(p[BB    ], x - 1, y - 1, z     ))),// FROM  8
                    newLerp(v, newLerp(u, grad(p[AA + 1], x    , y    , z - 1 ),  // CORNERS
                                          grad(p[BA + 1], x - 1, y    , z - 1 )), // OF CUBE
                               newLerp(u, grad(p[AB + 1], x    , y - 1, z - 1 ),
...
float newNoise(float x, float y, float z) {
  ...
  println(lerp2(w, lerp2(v, lerp2(u, grad(p[AA], x, y, z), grad(p[BA], x-1, y, z)),
      lerp2(u, grad(p[AB], x, y-1, z), grad(p[BB], x-1, y-1, z))),
      lerp2(v, lerp2(u, grad(p[AA+1], x, y, z-1), grad(p[BA+1], x-1, y, z-1)),
      lerp2(u, grad(p[AB+1], x, y-1, z-1), grad(p[BB+1], x-1, y-1, z-1)))) + 0.5);
